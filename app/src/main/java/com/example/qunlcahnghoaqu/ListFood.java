    ...
    private  void displayFruit() {
        ...
        if(cursor.getCount() == 0) {
            ...
            while (!cursor.isAfterLast()) {
                @SuppressLint("Range") String id = cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_ID_FRUIT));
                @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_NAME_FRUIT));
                @SuppressLint("Range") int count  = Integer.parseInt(cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_COUNT_FRUIT)));
                @SuppressLint("Range") Float price = Float.parseFloat(cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_PRICE_FRUIT)));
                @SuppressLint("Range") String category = cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_CATEGORY_FRUIT));
                @SuppressLint("Range") String producer = cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_PRODUCER_FRUIT));
                @SuppressLint("Range") byte[] img  = cursor.getBlob(cursor.getColumnIndex(DBHelper.COLUMN_IMG_FRUIT));
